  ...
  const extractNumberPlate = async (imageUrl: string) => {
    ...
    try {
      ...
      const {
        data,
        error
      } = await supabase.functions.invoke('detect-number-plate', {
        body: {
      ...
      <Card className="shadow-elevated">
        ...
        <CardContent className="p-6">
          <div className="space-y-6">
            {/* Dropbox Area */}
            <div onDrop={handleDrop} onDragOver={handleDragOver} onClick={() => fileInputRef.current?.click()} className="border-2 border-dashed border-muted-foreground/25 rounded-lg p-8 text-center hover:border-primary/50 transition-colors cursor-pointer bg-muted/5">
              {uploadedImage ? <div className="space-y-4">
